...
$classes = Flux::classes()
    ...
    ->add(match ($size) {
        default => 'text-base py-1.5 h-9 leading-none',
        'sm' => 'text-sm sm:text-sm py-2 h-8 leading-none', // This makes the height of the input 40px (same as buttons and such...)
        'xs' => 'text-xs py-1.5 h-6 leading-none',
    })
    ...
    <flux:with-field :$attributes :$name>
        <div {{ $attributes->only('class')->class('w-full relative block group/input') }} data-flux-input>
            <?php if (is_string($iconLeading)): ?>
                <div class="pointer-events-none absolute top-0 bottom-0 border-s border-transparent flex items-center justify-center text-xs text-zinc-400/75 ps-1.5 start-0">
                    <flux:icon :icon="$iconLeading" :variant="$iconVariant" :class="$iconClasses" />
                </div>
            <?php elseif ($iconLeading): ?>
                <div {{ $iconLeading->attributes->class('absolute top-0 bottom-0 border-s border-transparent flex items-center justify-center text-xs text-zinc-400/75 ps-1.5 start-0') }}>
                    {{ $iconLeading }}
                </div>
            <?php endif; ?>
